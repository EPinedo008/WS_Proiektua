...
function autocomplete(inp, game) {
    ...
    inp.addEventListener("input", function (e) {
        ...
        for (i = 0; i < players.length; i++) {
            let playerName = players[i].name; // Nombre completo
            let valLower = val.toLowerCase();

            // Verificar si alguna palabra empieza con lo ingresado
            ...
            if (matched) {
                ...
                // Resaltar coincidencias usando match y parse
                // Nota: autosuggest-highlight match es case-insensitive por defecto
                const indices = match(playerName, val); // encuentra posición de la coincidencia
